...
echo "[2/21] assembling kernel entry"
nasm -f elf64 "$KERNEL_ENTRY_SRC" -o "$KERNEL_ENTRY_OBJ"

echo "[3/21] assembling interrupt_stubs.asm"
...
echo "[4/21] compiling kernel_main.cpp"
"$CLANGXX_BIN" \
  --target=x86_64-elf \
...
echo "[5/21] compiling console.cpp"
"$CLANGXX_BIN" \
  --target=x86_64-elf \
  -I "$KERNEL_INCLUDE_DIR" \
  -ffreestanding \
...
echo "[7/21] compiling page_allocator.cpp"
"$CLANGXX_BIN" \
  --target=x86_64-elf \
  -I "$KERNEL_INCLUDE_DIR" \
  -ffreestanding \
...
echo "[8/21] compiling paging.cpp"
"$CLANGXX_BIN" \
  --target=x86_64-elf \
  -I "$KERNEL_INCLUDE_DIR" \
  -ffreestanding \
...
echo "[9/21] compiling runtime.cpp"
"$CLANGXX_BIN" \
  --target=x86_64-elf \
  -I "$KERNEL_INCLUDE_DIR" \
  -ffreestanding \
...
echo "[10/21] compiling interrupts.cpp"
"$CLANGXX_BIN" \
  --target=x86_64-elf \
  -I "$KERNEL_INCLUDE_DIR" \
  -ffreestanding \
...
echo "[13/21] compiling keyboard.cpp"
"$CLANGXX_BIN" \
  --target=x86_64-elf \
  -I "$KERNEL_INCLUDE_DIR" \
  -ffreestanding \
...
echo "[16/21] generating kernel.bin"
...
echo "[17/21] generating kernel metadata for stage2"
...
echo "[19/21] creating raw disk image"
...
echo "[20/21] verifying boot signature"
