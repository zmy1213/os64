...
if grep -q "stage1 ok" "$SERIAL_LOG" \
  ...
  && grep -q "sched_priority_pid=1" "$SERIAL_LOG" \
  && grep -q "sched_priority_high_tid=1" "$SERIAL_LOG" \
  && grep -q "sched_priority_normal_a_tid=2" "$SERIAL_LOG" \
  && grep -q "sched_priority_normal_b_tid=3" "$SERIAL_LOG" \
  && grep -q "sched_priority_background_tid=4" "$SERIAL_LOG" \
  && grep -q "sched_sleep_pid=2" "$SERIAL_LOG" \
  && grep -q "sched_sleep_thread_a_tid=5" "$SERIAL_LOG" \
  && grep -q "sched_sleep_thread_b_tid=6" "$SERIAL_LOG" \
  && grep -q "sched_block_pid=3" "$SERIAL_LOG" \
  && grep -q "sched_block_waiter_tid=7" "$SERIAL_LOG" \
  && grep -q "sched_block_waker_tid=8" "$SERIAL_LOG" \
  && grep -q "sched_priority_trace=HABABC" "$SERIAL_LOG" \
  && grep -q "sched_sleep_trace=ABAB" "$SERIAL_LOG" \
  && grep -q "sched_block_trace=BWb" "$SERIAL_LOG" \
  && grep -q "sched_priority_process_state=exited" "$SERIAL_LOG" \
  && grep -q "sched_sleep_process_state=exited" "$SERIAL_LOG" \
  && grep -q "sched_block_process_state=exited" "$SERIAL_LOG" \
  && grep -q "sched_idle_priority=idle" "$SERIAL_LOG" \
  && grep -q "sched_idle_dispatches=" "$SERIAL_LOG" \
  ...
  && grep -q "sched_sleeping_after=0" "$SERIAL_LOG" \
  && grep -q "sched_blocked_after=0" "$SERIAL_LOG" \
