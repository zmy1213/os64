...
## 8. 补充：为什么 boot volume 不能离内核太近

早期版本把 boot volume 预读到：
...
> 没有显式初始值的全局变量区域，内核启动时会被清成 0。

如果 bootloader 把 boot volume 放得太靠近内核，
就可能出现：

```text
stage2 先把 boot volume 读到内存
kernel 启动时清 .bss
.bss 把 boot volume 那块内存清成 0
```

这样文件系统就会读到一片全 0，
挂载自然失败。

所以现在构建脚本把 boot volume 预读地址放到：

```text
0x00080000
```

这个地址仍然在当前低 2 MiB 恒等映射范围内，
64 位内核可以直接访问；
同时它离内核镜像和 `.bss` 更远，
不容易被覆盖。
...
> bootloader 搬运数据时，要避开内核代码、数据和 bss 的增长空间。
