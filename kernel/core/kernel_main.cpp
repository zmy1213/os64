...
constexpr uint64_t kUserModePreemptSharedPageVirtualAddress =
    kUserModeCodeVirtualAddress + kPageSize;  // 再往后一页单独给“用户态自旋等待 timer 抢占”做共享标志页。
...
constexpr uint64_t kUserModeExpectedTimerPreemptResultFlags =
    kUserModeExpectedYieldResultFlags | kUserModeResultTimerPreemptOk;
constexpr uint64_t kUserModeExpectedStdinBlockResultFlags =
...
struct SchedulerUserYieldHelperContext {
  ...
  uint64_t run_count;               // helper 一共被调度到了多少次；现在既可能来自 `sys_yield`，也可能来自 timer 抢占。
  volatile uint64_t* shared_page;   // 指向那张“同时映射给用户态和内核态”的共享页；helper 会在这里给用户态打一个“你已经被抢占过了”的标志。
  uint64_t preempt_signal_count;    // helper 真正把“抢占完成”标志写进去多少次；这一步预期正好 1 次。
  ThreadControlBlock* wake_target;  // 后面再继续拿它观察：用户线程在 `read(0)` 空输入时，是否真的已经进入 blocked。
  uint8_t scancode;                 // helper 最后会注入这个测试扫描码，把阻塞中的用户线程从 keyboard wait queue 里唤醒。
  uint64_t stdin_signal_count;      // helper 真正注入“唤醒用户态 read(0)”的键盘扫描码多少次；当前预期正好 1 次。
  uint64_t stdin_block_observed_count;  // helper 在注入前，亲眼看到目标用户线程已经处于 blocked 状态多少次。
};
...
// 这一步把“用户程序从哪里来”单独抽成 helper。
// 以前用户代码直接来自内核镜像里的内嵌汇编字节；
// 现在我们想证明：OS64FS 里的普通文件也能变成用户代码页。
bool load_user_program_file(PageAllocator* allocator,
                            AddressSpace* user_space,
                            const Os64Fs* filesystem,
                            const char* path,
                            uint64_t virtual_address,
                            uint64_t* out_code_physical_page,
  ...
  if (allocator == nullptr ||
      ...
      filesystem == nullptr ||
      path == nullptr ||
      out_code_physical_page == nullptr ||
      out_inode_number == nullptr ||
      out_program_size == nullptr) {
    return false;
  }
  ...
  if (!file_handle_stat(&file_handle, &file_stat_result) ||
      ...
      file_stat_result.size_bytes > kPageSize) {
  ...
  *out_code_physical_page = code_physical_page;
  *out_inode_number = file_stat_result.inode_number;
  *out_program_size = file_stat_result.size_bytes;
  return true;
}
...
// 代码页不再来自 `user_mode_smoke_program_start..end`，
// 而是来自文件系统里的 `/docs/hello.bin`。
//
// 也就是说：
// 1. stage2 先把 boot volume 原始扇区预读进内存
// 2. 内核把那段内存挂成 BlockDevice
// 3. OS64FS 解析路径 `/docs/hello.bin`
// 4. FileHandle 把这个文件内容读到 1 张物理页里
// 5. 再把那张物理页映射成用户代码页，最后真的 iretq 进 ring 3
bool run_user_file_program_smoke_test(PageAllocator* allocator,
                                      const Os64Fs* filesystem,
                                      SyscallContext* context,
                                      FileDescriptorTable* fd_table) {
  if (allocator == nullptr ||
  ...
  uint64_t code_physical_page = 0;
  uint32_t program_inode_number = 0;
  size_t program_size = 0;
  if (!load_user_program_file(allocator, &user_space, filesystem,
                              kUserFileProgramPath,
                              kUserModeCodeVirtualAddress,
                              &code_physical_page,
                              &program_inode_number,
                              &program_size)) {
    return false;
  }
  ...
  serial_write_string("user_file_program_path=");
  serial_write_string(kUserFileProgramPath);
  serial_write_crlf();

  serial_write_string("user_file_program_inode=");
  serial_write_u64(program_inode_number);
  serial_write_crlf();

  serial_write_string("user_file_program_root=0x");
  serial_write_hex64(user_space.root_physical_address);
  serial_write_crlf();

  serial_write_string("user_file_program_code_phys=0x");
  serial_write_hex64(code_physical_page);
  serial_write_crlf();

  serial_write_string("user_file_program_stack_phys=0x");
  serial_write_hex64(stack_physical_page);
  serial_write_crlf();

  serial_write_string("user_file_program_entry=0x");
  serial_write_hex64(kUserModeCodeVirtualAddress);
  serial_write_crlf();

  serial_write_string("user_file_program_stack_top=0x");
  serial_write_hex64(kUserModeInitialStackPointer);
  serial_write_crlf();

  serial_write_string("user_file_program_size=");
  serial_write_u64(program_size);
  serial_write_crlf();
  ...
  serial_write_string("user_file_program_return_cs=0x");
  serial_write_hex64(return_cs);
  serial_write_crlf();

  serial_write_string("user_file_program_return_cpl=");
  serial_write_u64(return_cs & 0x3);
  serial_write_crlf();

  serial_write_string("user_file_program_return_flags=0x");
  serial_write_hex64(return_flags);
  serial_write_crlf();

  const bool ok =
      program_inode_number == 6 &&
      return_cs == kUserModeExpectedCodeSelector &&
      (return_cs & 0x3) == kUserModeExpectedCpl &&
      return_flags == kUserModeExpectedFileProgramResultFlags;
  if (ok) {
    serial_write_string("user_file_program ok");
    serial_write_crlf();
  }
...
bool run_filesystem_smoke_test(const BlockDevice* device,
  ...
  serial_write_string("os64fs_hello_inode=");
...
void scheduler_user_yield_helper_thread_entry(void* raw_context) {
  ...
  for (;;) {
    ...
    // 这条 helper thread 现在不再只是“被 yield 到一次就结束”。
    // 它会一直活着，直到看到共享页里的 arm 标志被用户态置 1：
    // - arm == 0：说明用户态还没进入“等待 timer 抢占”的那一段，helper 只先让出 CPU
    // - arm == 1：说明用户态已经进入自旋等待；这时 helper 把 done 置 1，告诉用户态“你确实被切走又切回来了”
    if (context->shared_page != nullptr &&
        context->shared_page[0] != 0 &&
        context->shared_page[1] == 0) {
      context->shared_page[1] = 1;
      ++context->preempt_signal_count;
      (void)scheduler_yield_current_thread();
      continue;
    }

    // 第二阶段再继续证明：
    // user thread 在 ring3 调 `read(0)` 时，如果当前没有字符，确实会先 block；
    // helper 必须先看到它已经 blocked，再去模拟键盘 IRQ，把它真正唤醒。
    if (context->shared_page != nullptr &&
        context->shared_page[2] != 0 &&
        context->shared_page[3] == 0) {
      if (context->wake_target == nullptr ||
          context->wake_target->state != kThreadStateBlocked) {
        (void)scheduler_yield_current_thread();
        continue;
      }
...
bool run_scheduler_user_thread_smoke_test(PageAllocator* allocator,
  ...
  if (!address_space_map_user_page(&process->address_space, allocator,
                                   kUserModePreemptSharedPageVirtualAddress,
                                   shared_physical_page,
                                   kPageWritable)) {
    return false;
  }
  ...
  // 这次必须把 IF 打开。
  // 原因是我们后面要验证“用户线程正在 ring3 跑时，真的会收到 timer IRQ 并被抢占”。
  // 如果 IF 还是关着，用户态就永远接不到可屏蔽外部中断，这一轮测试根本不会发生。
  ...
  serial_write_string("user_thread_preempt_shared_phys=0x");
  serial_write_hex64(shared_physical_page);
  serial_write_crlf();
  ...
  serial_write_string("user_thread_preempt_shared_virt=0x");
  serial_write_hex64(kUserModePreemptSharedPageVirtualAddress);
  serial_write_crlf();
  ...
  const uint64_t stdin_done_flag = shared_page[3];
  ...
  serial_write_string("user_thread_kernel_root=0x");
  serial_write_hex64(kernel_root_physical);
  serial_write_crlf();
  ...
  serial_write_string("user_thread_helper_stack_base=0x");
  serial_write_hex64(
      reinterpret_cast<uint64_t>(helper_thread->stack_allocation));
  serial_write_crlf();

  serial_write_string("user_thread_helper_resume_root=0x");
  serial_write_hex64(helper_thread->saved_address_space_root_physical);
  serial_write_crlf();

  serial_write_string("user_thread_helper_preempt_signals=");
  serial_write_u64(helper_context.preempt_signal_count);
  serial_write_crlf();
  ...
  serial_write_string("user_thread_total_yield_count=");
  serial_write_u64(thread->yield_count);
  serial_write_crlf();

  serial_write_string("user_thread_preempt_count=");
  serial_write_u64(thread->user_preempt_count);
  serial_write_crlf();

  serial_write_string("user_thread_preempt_arm_flag=");
  serial_write_u64(preempt_arm_flag);
  serial_write_crlf();

  serial_write_string("user_thread_preempt_done_flag=");
  serial_write_u64(preempt_done_flag);
  serial_write_crlf();

  serial_write_string("user_thread_stdin_arm_flag=");
  serial_write_u64(stdin_arm_flag);
  serial_write_crlf();

  serial_write_string("user_thread_stdin_done_flag=");
  serial_write_u64(stdin_done_flag);
  serial_write_crlf();

  serial_write_string("user_thread_helper_stdin_signals=");
  serial_write_u64(helper_context.stdin_signal_count);
  serial_write_crlf();

  serial_write_string("user_thread_helper_block_observed=");
  serial_write_u64(helper_context.stdin_block_observed_count);
  serial_write_crlf();
  ...
  serial_write_string("user_thread_preempt_trap_vector=0x");
  serial_write_hex64(thread->user_preempt_trap_frame.vector);
  serial_write_crlf();

  serial_write_string("user_thread_preempt_trap_cs=0x");
  serial_write_hex64(thread->user_preempt_trap_frame.cs);
  serial_write_crlf();

  serial_write_string("user_thread_preempt_trap_ss=0x");
  serial_write_hex64(thread->user_preempt_trap_frame.ss);
  serial_write_crlf();

  serial_write_string("user_thread_preempt_trap_rip=0x");
  serial_write_hex64(thread->user_preempt_trap_frame.rip);
  serial_write_crlf();

  serial_write_string("user_thread_preempt_trap_rsp=0x");
  serial_write_hex64(thread->user_preempt_trap_frame.rsp);
  serial_write_crlf();
  ...
  const bool ok =
      ...
      return_cs == kUserModeExpectedCodeSelector &&
      (return_cs & 0x3) == kUserModeExpectedCpl &&
      return_flags == kUserModeExpectedStdinBlockResultFlags &&
      ...
      helper_context.run_count >= 3 &&
      helper_thread->stack_allocation != nullptr &&
      reinterpret_cast<uint64_t>(helper_thread->stack_allocation) >=
          kPagingBootIdentityLimit &&
      helper_thread->saved_address_space_root_physical ==
          kernel_root_physical &&
      helper_context.preempt_signal_count == 1 &&
      helper_context.stdin_signal_count == 1 &&
      helper_context.stdin_block_observed_count == 1 &&
      thread->user_yield_count == 1 &&
      thread->yield_count >= 2 &&
      thread->has_user_preempt_trap_frame &&
      thread->user_preempt_count >= 1 &&
      preempt_arm_flag == 1 &&
      preempt_done_flag == 1 &&
      stdin_arm_flag == 1 &&
      stdin_done_flag == 1 &&
      ...
      thread->user_preempt_trap_frame.vector == kPicMasterVectorBase &&
      thread->user_preempt_trap_frame.cs == kUserCodeSelectorRpl3 &&
      thread->user_preempt_trap_frame.ss == kUserDataSelectorRpl3 &&
      thread->user_preempt_trap_frame.rip >= kUserModeCodeVirtualAddress &&
      thread->user_preempt_trap_frame.rsp >= kUserModeStackPageVirtualAddress &&
      thread->user_preempt_trap_frame.rsp <= kUserModeInitialStackPointer &&
...
extern "C" void kernel_main(const BootInfo* boot_info) {
  ...
  if (!run_user_file_program_smoke_test(&g_page_allocator,
                                        &g_os64fs,
                                        &g_syscall_context,
                                        &g_fd_table)) {
    write_status_line(kStorageStatusRow, "user file bad");
    return;
  }
