...
%define FLOPPY_SECTORS_PER_TRACK 18   ; 1.44MB 软盘镜像每个磁道 18 个扇区。
%define FLOPPY_HEADS             2    ; 每个柱面有 2 个磁头。
...
boot_volume_loaded_message db 'boot volume loaded ok', 0
                                       ; 说明给内核演示用的启动卷也已经被预读到了内存。
...
boot_volume_load_error db 'boot volume load failed', 0
                                       ; 读启动卷失败时的提示。

boot_drive             db 0                          ; 保存 BIOS 传来的启动盘号，后面 int 13h 读内核要用。
disk_load_remaining    dw 0                          ; 通用读盘例程里，当前还有多少个扇区没读完。
disk_load_offset       dw 0                          ; 当前这次 BIOS 应该写到 ES:BX 的哪个偏移。
disk_load_lba          dw 0                          ; 当前要读的“线性扇区号（从 0 开始）”。
disk_load_segment      dw 0                          ; 当前这段扇区应该被读到哪个段地址。
...
boot_info:
    ...
    dq BOOT_VOLUME_LOAD_ADDR           ; 再把 boot volume 预读到哪块内存也告诉内核。
    dd BOOT_VOLUME_START_SECTOR - 1    ; 内核看到的起始扇区号统一用从 0 开始的 LBA。
    dw BOOT_VOLUME_SECTORS             ; 一共给内核预读了多少个扇区。
    dw 512                             ; 这一步先固定成 512 字节扇区。
...
start:                                 ; stage2 的 16 位实模式入口。
    ...
    call load_boot_volume_from_disk    ; 再把给 64 位内核演示用的启动卷一起预读进内存。
    jc stage2_boot_volume_load_failed  ; 如果这一步失败，后面的“按扇区读取”链就不存在了。

    mov si, boot_volume_loaded_message ; 现在再明确告诉你：卷数据也已经准备好了。
    call print_string_screen
    mov si, boot_volume_loaded_message
    call print_string_serial
    call print_crlf_serial
...
stage2_boot_volume_load_failed:        ; 读启动卷失败时的错误分支。
    mov si, boot_volume_load_error     ; SI 指向错误字符串。
    call print_string_screen           ; 屏幕打印错误。
    mov si, boot_volume_load_error     ; 串口也打印错误。
    call print_string_serial
    call print_crlf_serial
...
load_kernel_from_disk:                 ; 第一份要读的是 kernel.bin。
    xor ax, ax                         ; 先确保下面这些状态变量一定写回 stage2 自己的 0 段。
    mov ds, ax
    mov word [disk_load_remaining], KERNEL_SECTORS
                                       ; 总共要读多少个内核扇区。
    mov word [disk_load_offset], KERNEL_LOAD_OFFSET
                                       ; 这段数据从目标段的哪个偏移开始写。
    mov word [disk_load_lba], KERNEL_START_SECTOR - 1
                                       ; 从第几个线性扇区开始读。
    mov word [disk_load_segment], KERNEL_LOAD_SEGMENT
                                       ; 目标段固定是 0x1000，对应 0x00010000。
    call load_disk_range               ; 真正走通用读盘例程。
    ret

load_boot_volume_from_disk:            ; 第二份要读的是给内核演示用的 boot volume。
    xor ax, ax                         ; 同样先确保状态变量写回 0 段。
    mov ds, ax
    mov word [disk_load_remaining], BOOT_VOLUME_SECTORS
                                       ; 这一步目前固定只读一个很小的卷。
    mov word [disk_load_offset], BOOT_VOLUME_LOAD_OFFSET
                                       ; 从目标段偏移 0 开始写。
    mov word [disk_load_lba], BOOT_VOLUME_START_SECTOR - 1
                                       ; 它紧跟在 kernel.bin 后面。
    mov word [disk_load_segment], BOOT_VOLUME_LOAD_SEGMENT
                                       ; 先把它放到低 2 MiB 恒等映射区里，内核能直接访问。
    call load_disk_range               ; 同样复用通用读盘例程。
    ret

load_disk_range:                       ; 用最基础的 BIOS int 13h 读一段连续扇区到固定内存。
                                       ; 这里仍然坚持 CHS 读盘，但为了先求稳、先讲清楚，
                                       ; 我们改成“每次只读 1 个扇区”：
                                       ; 1. 记住当前线性扇区号 LBA
                                       ; 2. 每轮把 LBA 重新换算成 cylinder/head/sector
                                       ; 3. 读 1 个扇区到 ES:BX
                                       ; 4. LBA 和内存偏移都往后推进 512 字节
                                       ; 这样内核、启动卷两种数据都能共用同一条读盘主路。
    ...
    xor ax, ax                         ; 前面 BIOS 调用不保证 DS 还保持 0，
    mov ds, ax                         ; 所以一进入这里就先把 DS 拉回 0 段。
    mov ax, [disk_load_segment]        ; AX = 目标段地址，可能是内核，也可能是启动卷。
    mov es, ax                         ; ES:BX 就是 BIOS int 13h 的目标缓冲区地址。

.loop:
    xor ax, ax                         ; 某些 BIOS 调完 int 13h 后不保证 DS 还保持原值，
    mov ds, ax                         ; 所以每轮一开始都把 DS 拉回 0，确保下面这些 [disk_load_*]
                                       ; 变量访问的还是 stage2 自己这一段内存，而不是飘到别的段里。
    cmp word [disk_load_remaining], 0
    je .success                        ; 没有剩余扇区了，说明这段数据已经都读完了。

    mov ax, [disk_load_lba]            ; AX = 当前线性扇区号，从 0 开始。
    xor dx, dx                         ; DX:AX 准备做 16 位除法。
    mov cx, FLOPPY_SECTORS_PER_TRACK * FLOPPY_HEADS
                                       ; CX = 36，表示“每个柱面总共有 36 个扇区”。
    div cx                             ; AX = cylinder，DX = 这个柱面内部的余数（0~35）。
    mov bx, ax                         ; BX 先暂存 cylinder。
                                       ; 这里不能直接先写进 CH，
                                       ; 因为后面为了再算 head/sector 还要改写整个 CX，
                                       ; 如果现在就写 CH，后面的 `mov cx, 18` 会把 CH 一起覆盖掉。

    mov ax, dx                         ; AX = 柱面内部余数，再继续拆成 head 和 sector。
    xor dx, dx                         ; DX:AX 再准备下一次除法。
    mov cx, FLOPPY_SECTORS_PER_TRACK
                                       ; CX = 18，表示“每个磁头各有 18 个扇区”。
    div cx                             ; AX = head，DX = 这个磁头内的扇区偏移（0~17）。
    mov ch, bl                         ; CH = cylinder 低 8 位。
                                       ; 现在 head/sector 已经算完了，再把 cylinder 放回去就不会被覆盖。
    mov dh, al                         ; DH = 磁头号，0 或 1。
    mov cl, dl                         ; CL 先放扇区偏移。
    inc cl                             ; BIOS CHS 的扇区编号从 1 开始，所以这里要 +1。

    mov ax, [disk_load_segment]        ; 有些 BIOS 调用后不保证 ES 完全保持原样，所以每一轮都重新装一遍更稳。
    mov es, ax                         ; 这样可以确保当前这批扇区一定写到我们期望的目标段里。
    mov bx, [disk_load_offset]         ; BX = 这次 BIOS 应该把数据写到 ES:BX 的哪个偏移。
    mov ah, 0x02                       ; AH = 0x02，表示 BIOS 传统“读扇区”功能。
    mov al, 0x01                       ; AL = 1，这一版每轮只读 1 个扇区，先把正确性做稳。
    mov dl, [boot_drive]               ; DL = 启动盘号，告诉 BIOS 从哪块盘继续读。
    int 0x13                           ; 真正发起本轮读盘。
    jc .failed                         ; 如果 CF=1，说明 BIOS 读这批扇区失败。

    xor ax, ax                         ; BIOS 返回后，再把 DS 重装一次。
    mov ds, ax                         ; 不然下面更新 remaining/offset/sector 时，
                                       ; 这些内存变量也可能写到了错误的段里。
    mov ax, [disk_load_offset]         ; AX = 当前偏移。
    add ax, 512                        ; 下一扇区应该写到后面 512 字节的位置。
    mov [disk_load_offset], ax         ; 先把低 16 位偏移写回去。
    jnc .offset_ok                     ; 如果没有回卷，说明还在同一个段里继续写就行。
    add word [disk_load_segment], 0x1000
                                       ; 一旦偏移跨过 64 KiB，就要把段地址前移 0x1000。
                                       ; 原因是实模式线性地址 = segment * 16 + offset。
                                       ; offset 从 0xFE00 再加 0x0200 会回卷到 0，
                                       ; 这时 segment 必须一起前进 0x1000，线性地址才会连续。
.offset_ok:
    inc word [disk_load_lba]           ; 线性扇区号 +1，下一轮重新换算新的 CHS。
    dec word [disk_load_remaining]     ; 总剩余扇区数 -1。
    jmp .loop                          ; 继续读下一扇区。

.success:
    clc                                ; CF = 0，表示整个内核都成功读进内存了。
