...
kernel_load_remaining  dw 0                          ; 还剩多少个内核扇区没读完。
kernel_load_offset     dw 0                          ; 当前应该写到 ES:BX 里的哪个偏移。
kernel_load_lba        dw 0                          ; 当前要读的“线性扇区号（从 0 开始）”。
...
load_kernel_from_disk:                 ; 用最基础的 BIOS int 13h 读盘，把 kernel.bin 读到固定地址。
    ...
    mov ds, ax                         ; 所以一进入这里就先把 DS 拉回 0 段。
                                       ; 不然下面初始化 kernel_load_remaining / offset / lba
                                       ; 这几个变量时，可能会写进错误的段里。
    ...
    mov word [kernel_load_remaining], KERNEL_SECTORS
                                       ; 先记下总共还有多少个扇区要读。
    mov word [kernel_load_offset], KERNEL_LOAD_OFFSET
                                       ; 当前写入偏移从 0 开始，也就是 ES:0000。
    mov word [kernel_load_lba], KERNEL_START_SECTOR - 1
                                       ; BIOS CHS 的扇区号从 1 开始，但我们内部先用从 0 开始的线性扇区号更好算。

.loop:
    xor ax, ax                         ; 某些 BIOS 调完 int 13h 后不保证 DS 还保持原值，
    mov ds, ax                         ; 所以每轮一开始都把 DS 拉回 0，确保下面这些 [kernel_load_*]
                                       ; 变量访问的还是 stage2 自己这一段内存，而不是飘到别的段里。
    cmp word [kernel_load_remaining], 0
    je .success                        ; 没有剩余扇区了，说明整个 kernel.bin 都读完了。

    mov ax, [kernel_load_lba]          ; AX = 当前线性扇区号，从 0 开始。
    ...
    mov ax, KERNEL_LOAD_SEGMENT        ; 有些 BIOS 调用后不保证 ES 完全保持原样，所以每一轮都重新装一遍更稳。
    mov es, ax                         ; 这样可以确保当前这批扇区一定写到我们期望的目标段里。
    mov bx, [kernel_load_offset]       ; BX = 这次 BIOS 应该把数据写到 ES:BX 的哪个偏移。
    ...
    mov ds, ax                         ; 不然下面更新 remaining/offset/sector 时，
                                       ; 这些内存变量也可能写到了错误的段里。
    add word [kernel_load_offset], 512 ; 下一扇区应该写到后面 512 字节的位置。
    inc word [kernel_load_lba]         ; 线性扇区号 +1，下一轮重新换算新的 CHS。
    dec word [kernel_load_remaining]   ; 总剩余扇区数 -1。
    jmp .loop                          ; 继续读下一扇区。
